...
function Page1() {
  ...
  const type = params.get("type"); // "buy" or "rent"
  const searchLocationFromURL = params.get("location") || "Palm Jumeirah"; 

  const [searchInput, setSearchInput] = useState(searchLocationFromURL);

  useEffect(() => {
    const els = document.querySelectorAll(".page1-animate-on-scroll");
  ...
  useEffect(() => {
    fetch(`http://localhost:5000/properties?type=${type}`)
      .then((res) => res.json())
  ...
  return (
    <div className="page1-wrapper">
      ...
      <nav className="page1-navbar">
        <div className="page1-nav-logo">ORTUS</div>
      ...
      <section className="page1-hero">
        <img
          ...
          className="page1-hero-image"
        />
        <div className="page1-hero-overlay">
          {/* SEARCH BAR */}
          <div className="page1-searchbar">
            <div className="searchbar-input-wrapper">
              <input
                type="text"
                placeholder={searchLocationFromURL}
                className="searchbar-input"
                value={searchInput}
                onChange={(e) => setSearchInput(e.target.value)}
              />
              <button className="searchbar-icon">🔍</button>
            </div>
      ...
      {/* BREADCRUMB + FILTERS */}
      <section className="page1-breadcrumb-filters">
        <div className="page1-breadcrumb-left">
          <a href="#">Home</a> » 
          <a href="#">{`Property for ${type === "buy" ? "sale" : "rent"} in Dubai`}</a> » 
          <a href="#">{`Property for ${type === "buy" ? "sale" : "rent"} in ${searchInput}`}</a>
        </div>

        <div className="page1-filters-right">
          <span className="page1-property-size-label">
            <i className="icon-user"></i> Property size
          </span>
          <button className="page1-size-btn">SQ.M</button>
          <button className="page1-size-btn active">SQ.FT</button>
          <span className="page1-sort-by">Sort by</span>
          <button className="page1-sort-dropdown">▼</button>
        </div>
      </section>

      {/* PROPERTY LISTINGS */}
      <section className="page1-property-listings">
        <div className="property-grid">
          {properties.length === 0 && <p>No properties available.</p>}
          {properties.map((p, i) => (
            <div className="property-card" key={i}>
              <div className="property-image">
                <img src={p.images[0] || "/images/sample-property.png"} alt={p.title} />
                <span className={`property-type-badge ${type}`}>{type === "buy" ? "For Buy" : "For Rent"}</span>
